    ...
    where TEntity : IEntity<TEntity>
{
    ...
    {
        var agent = await _agentAccessor.GetAgentAsync(agentSignatureOptionsName);

        return new TransactionBuilder<TEntity>(agent);
    }

    public async Task<ISingleEntityTransactionBuilder<TEntity>> CreateForSingleEntity(string agentSignatureOptionsName, Id entityId)
    {
        var transactionBuilder = await Create(agentSignatureOptionsName);
